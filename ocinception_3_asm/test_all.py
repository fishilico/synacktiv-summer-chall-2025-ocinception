...
BUILD_CONFIGURATIONS: tuple[tuple[str, str, str], ...] = (
    ("32_none_zstd", "i686-unknown-none", ""),
    ("32_none_gzip", "i686-unknown-none", "gzip"),
    ("32_optim_zstd", "i686-unknown-optim", ""),
    ("32_optim_gzip", "i686-unknown-optim", "gzip"),
)
...
if not arg_notest:
    ...
    for rust_target in RUST_TEST_TARGETS:
        for rust_features in ("with-debug,gzip", "gzip"):
...
for config_name, rust_target, rust_features in BUILD_CONFIGURATIONS:
    ...
    subprocess.run(
        [
            ...
            "--features",
            rust_features,
            "-Zbuild-std=core" + (",alloc" if "gzip" in rust_features else ""),
        ]
    ...
    compiled_program_path = target_dir_path / "ocinception_3"
    if rust_target == "i686-unknown-optim":
        # Do not strip the already-optimized binary
        program_path = compiled_program_path
    else:
        program_path = target_dir_path / "ocinception_3_strip"
